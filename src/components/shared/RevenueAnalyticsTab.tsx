...
// ─── Import Expense type from service ──────────────────────────────────
...
const RevenueAnalyticsTab: React.FC<RevenueAnalyticsTabProps> = ({
  ...
  const invoiceService = new InvoiceService('', 'superadmin');
  ...
  const fetchExpenses = async (): Promise<Expense[]> => {
    try {
      const data = await expenseServiceInstance.getExpenses();
      return data || [];
    } catch (error) {
  ...
  const enhanceSiteProfitsWithSiteData = (sitesData: Site[]) => {
    ...
    return siteProfits.map(profit => {
      ...
      return {
        ...
        clientName: site?.clientName || 'Client not available',
        siteType: site?.status || 'Active'
    ...
    expenses.forEach(expense => {
      if (expense.status === 'approved') {
        ...
        if (!categoryMap[category]) {
          categoryMap[category] = { value: 0, count: 0, site: 'Unknown Site' };
        }
        categoryMap[category].value += expense.amount;
        categoryMap[category].count += 1;
        // Handle siteId (could be string or object)
        if (expense.siteId) {
          const siteName = typeof expense.siteId === 'object' && 'name' in expense.siteId
            ? expense.siteId.name
            : String(expense.siteId);
          if (siteName && siteName !== 'Unknown Site') {
            categoryMap[category].site = siteName;
          }
        }
      }
    });
  ...
  const calculateSummaryStats = () => {
    ...
    const expenseSites = [...new Set(expenses.map(exp => {
      if (typeof exp.siteId === 'object' && 'name' in exp.siteId) {
        return exp.siteId.name;
      }
      return exp.siteId || '';
    }).filter(Boolean))];
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg text-xs">
          ...
          <p className="mt-1">{`Amount: ${formatCurrency(data.value || data.revenue || 0)}`}</p>
                      ...
                      <div key={expense._id} className="flex justify-between items-center p-1.5 hover:bg-gray-50 rounded text-xs">
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate text-xs">
                            {typeof expense.siteId === 'object' && 'name' in expense.siteId
                              ? expense.siteId.name
                              : expense.siteId || 'Unknown Site'}
                          </p>
