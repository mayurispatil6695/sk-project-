...
const SupervisorDashboard = () => {
  ...
  const [shiftSelectionOpen, setShiftSelectionOpen] = useState(false);
  ...
  const fetchShiftsForSite = useCallback(async (siteNameOrId: string) => {
    if (!siteNameOrId) return [];
    setLoadingShifts(true);
    try {
      const sites = await siteService.getAllSites();
      let site = sites.find(s => s.name === siteNameOrId);
      if (!site) site = sites.find(s => s._id === siteNameOrId);

      const shifts = site?.shifts && site.shifts.length > 0 ? site.shifts : [];
      setSiteShifts(shifts);
      if (shifts.length > 0) {
      ...
      return shifts; // ✅ return the fresh data
    } catch (error) {
  ...
  useEffect(() => {
    if (selectedSite) {
      fetchShiftsForSite(selectedSite);
    }
  }, [selectedSite, fetchShiftsForSite]);
  // Auto‑refresh shifts when a site is updated (event from SitesSection)
  useEffect(() => {
    const handleSiteUpdated = () => {
      if (selectedSite) {
        console.log('🔄 Site updated – re‑fetching shifts for', selectedSite);
        fetchShiftsForSite(selectedSite);
      }
    };
    window.addEventListener('siteUpdated', handleSiteUpdated);
    return () => window.removeEventListener('siteUpdated', handleSiteUpdated);
  }, [selectedSite, fetchShiftsForSite]);
  const handlePhotoCapture = async (photoFile: File) => {
    ...
    try {
      ...
      formData.append('photo', photoFile);
      formData.append('supervisorId', currentSupervisor.id);
      formData.append('shiftId', selectedShiftId || '');   // <-- ADD THIS
  ...
  const handleRefresh = () => {
    ...
    // ✅ ADD THIS – reload shifts for the current site
    if (selectedSite) {
      fetchShiftsForSite(selectedSite);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <Dialog open={shiftSelectionOpen} onOpenChange={setShiftSelectionOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Select Shift for Attendance</DialogTitle>
            <DialogDescription>
              Choose the shift for the current attendance session. This will apply to all check-ins/outs.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {loadingShifts ? (
              ...
              <div className="text-center text-muted-foreground">
                No shifts defined for this site. Please define shifts in site settings.
              </div>
            ) : (
              <Select
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShiftSelectionOpen(false)}
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={!selectedShiftId || loadingShifts}
            >
