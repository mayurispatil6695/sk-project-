...
// Type for imported site row
type ImportedSiteRow = Record<string, string>;

// Type for validated site object (matches CreateSiteRequest but without clientId)
interface ValidatedSite {
  name: string;
  clientName: string;
  location: string;
  areaSqft: number;
...
const SitesSection = () => {
  ...
  const [importPreview, setImportPreview] = useState<ImportedSiteRow[]>([]);
  ...
  const fetchSites = async () => {
    try {
      ...
      setError(msg || "Failed to load sites");
  ...
  const searchSites = async () => {
    try {
      ...
      const msg = getErrorMessage(error);
      setError(msg || "Failed to search sites");
      toast.error(msg || "Failed to search sites");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddOrUpdateSite = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (msg?.includes('Duplicate entry') || msg?.includes('duplicate')) {
        ...
        toast.error(msg || "Failed to save site");
      }
    }
  };
  ...
  const handleToggleStatus = async (siteId: string) => {
    try {
      ...
      toast.error(getErrorMessage(error) || "Failed to update site status");
    }
  };
  ...
  // Read Excel file – properly typed
  const readExcelFile = (file: File): Promise<ImportedSiteRow[]> => {
  ...
  const validateImportedSites = async (importedData: ImportedSiteRow[]): Promise<ValidationResults> => {
    const validSites: ValidatedSite[] = [];
    const invalidSites: ImportedSiteRow[] = [];
    ...
    for (let index = 0; index < importedData.length; index++) {
      ...
      // Parse staff deployment – use const
      const staffDeployment: Array<{ role: string; count: number }> = [];
      StaffRoles.forEach(role => {
        const roleKey = role.replace(/\s+/g, '');
        const roleCount = row[role] || row[roleKey] || row[role.toUpperCase()] || '';
        const count = parseInt(String(roleCount)) || 0;
        if (count > 0) {
  ...
  const handleImportSites = async () => {
    ...
    for (const siteData of validationResults.valid) {
      try {
        ...
        const msg = getErrorMessage(error);
        errors.push(`${siteData.name}: ${msg || 'Unknown error'}`);
        errorCount++;
      }
    }
