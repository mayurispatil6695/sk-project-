...
const AlertsSection = () => {
  ...
  const fetchWorkQueries = async () => {
    ...
    try {
     ...
     const err = error as { response?: { status?: number; data?: unknown }; code?: string };
      console.error('Error details:', err.response?.data || getErrorMessage(error));
  ...
  const handleUpdateStatus = async (alertId: string, status: Alert["status"]) => {
    try {
      ...
      toast.error(getErrorMessage(error) || 'Failed to update alert status');
    }
  };
  ...
  const handleAddAlert = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(getErrorMessage(error) || 'Failed to create alert. Check console for details.');
    } finally {
      setSubmitting(false);
    }
  };
