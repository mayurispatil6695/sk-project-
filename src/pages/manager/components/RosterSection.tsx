...
import {
  Calendar as CalendarIcon, Plus, Calendar, Download, Edit, Trash2, ChevronLeft, ChevronRight,
  Loader2, RefreshCw, User, Clock, X, ChevronDown, ChevronUp, Filter, Users, CalendarDays,
  CalendarRange, CalendarCheck, Search, Eye, Info, Building, UserCog
} from "lucide-react";
import { format, addMonths, subMonths, startOfMonth, endOfMonth, eachDayOfInterval, startOfWeek, endOfWeek, isSameMonth, isSameDay, addDays, differenceInDays, getWeek, getYear } from "date-fns";
...
const ManagerRosterSection = () => {
  ...
  // Old Add Entry stuff (kept for manager to assign tasks/shifts)
  ...
  const [newRosterEntry, setNewRosterEntry] = useState<RosterEntryData>({
    date: format(new Date(), "yyyy-MM-dd"),
    employeeName: "", employeeId: "", department: "", designation: "", shift: "",
    shiftTiming: "", assignedTask: "", assignedTaskId: "", hours: 8, remark: "",
    type: "daily", siteClient: "", siteId: "", supervisors: [], managers: []
  });
  const [loadingData, setLoadingData] = useState({ sites: true, supervisors: true, managers: true, employees: true, tasks: true });
  ...
  useEffect(() => {
    if (managerId && isAuthenticated) fetchManagerAssignedSites();
  }, [managerId, isAuthenticated, fetchManagerAssignedSites]);

  // Load Sites filtered by manager
  useEffect(() => {
    if (!managerId) return;
    (async () => {
      try {
    ...
    try {
      if (isDefault) {
        ...
        const res = await rosterService.createRosterEntry({
          ...
          siteClient: selectedSite.name, siteId: selectedSite._id, supervisors: [], managers: [], createdBy: managerId
        });
        if (!res.success) throw new Error(res.message || "Failed to update");
      ...
      <Card>
        <CardHeader className="p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2"><Calendar className="h-5 w-5 md:h-6 md:w-6" /><CardTitle className="text-lg md:text-xl">Manager Roster Management</CardTitle></div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={fetchRosterEntries} disabled={isLoading}><RefreshCw className={cn("mr-2 h-4 w-4", isLoading && "animate-spin")} /> Refresh</Button>
              <Button onClick={handleExportReport} disabled={isLoading}><Download className="mr-2 h-4 w-4" /> Export</Button>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-4 md:p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {[{ value: "daily", label: "Daily", icon: Calendar }, { value: "weekly", label: "Weekly", icon: CalendarDays }, { value: "fortnightly", label: "Fortnightly", icon: CalendarRange }, { value: "monthly", label: "Monthly", icon: CalendarCheck }].map((type) => {
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-end">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="icon" onClick={() => setSelectedDate(prev => { if (selectedRoster === "daily") return addDays(prev, -1); if (selectedRoster === "weekly") return addDays(prev, -7); if (selectedRoster === "fortnightly") return addDays(prev, -14); return subMonths(prev, 1); })}><ChevronLeft className="h-4 w-4" /></Button>
                <div className="w-36 text-center font-medium text-sm border rounded-md h-10 flex items-center justify-center">{selectedRoster === "daily" ? format(selectedDate, "dd MMM yyyy") : selectedRoster === "weekly" ? `Week of ${format(daysInView[0], "dd MMM")}` : selectedRoster === "fortnightly" ? `Fortnight of ${format(daysInView[0], "dd MMM")}` : format(selectedDate, "MMMM yyyy")}</div>
                <Button variant="outline" size="icon" onClick={() => setSelectedDate(prev => { if (selectedRoster === "daily") return addDays(prev, 1); if (selectedRoster === "weekly") return addDays(prev, 7); if (selectedRoster === "fortnightly") return addDays(prev, 14); return addMonths(prev, 1); })}><ChevronRight className="h-4 w-4" /></Button>
              </div>
            </div>

            <div className="flex-1 md:max-w-xs">
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Search Employee</label>
              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Name or ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 h-10" /></div>
            </div>
          </div>
                  ...
                  {filteredEmployees.map((emp) => {
                    ...
                    return (
                      <TableRow key={emp._id}>
                        <TableCell className="sticky left-0 bg-background z-10"><div className="font-medium text-sm">{emp.name}</div><div className="text-xs text-muted-foreground">{emp.employeeId}</div></TableCell>
                        <TableCell>
                          <Select value={emp.assignedWeekOff || "none"} onValueChange={(val) => handleAssignedWeekOffChange(emp, val === "none" ? "" : val)}>
                            <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="None" /></SelectTrigger>
                            <SelectContent><SelectItem value="none">None</SelectItem>{WEEK_DAYS.map(day => (<SelectItem key={day} value={day}>{day}</SelectItem>))}</SelectContent>
                          </Select>
                        </TableCell>
                        {daysInView.map((day, idx) => {
